...
class ComplexLogistic(object):
    def __init__(self, no_subjects, no_objects, no_relations, embedding_size, l2_reg_lambda):
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            self.e1_sub = tf.reshape(tf.nn.embedding_lookup(e1, tf.gather(tf.transpose(self.input_x), [0])),
                                     tf.stack([self.batch_size_negs, embedding_size]))
            self.e1_obj = tf.reshape(tf.nn.embedding_lookup(e1, tf.gather(tf.transpose(self.input_x), [2])),
                                     tf.stack([self.batch_size_negs, embedding_size]))
            self.e2_sub = tf.reshape(tf.nn.embedding_lookup(e2, tf.gather(tf.transpose(self.input_x), [0])),
                                     tf.stack([self.batch_size_negs, embedding_size]))
            self.e2_obj = tf.reshape(tf.nn.embedding_lookup(e2, tf.gather(tf.transpose(self.input_x), [2])),
                                     tf.stack([self.batch_size_negs, embedding_size]))
            self.r1 = tf.reshape(tf.nn.embedding_lookup(r1, tf.gather(tf.transpose(self.input_x), [1])),
                                 tf.stack([self.batch_size_negs, embedding_size]))
            self.r2 = tf.reshape(tf.nn.embedding_lookup(r2, tf.gather(tf.transpose(self.input_x), [1])),
                                 tf.stack([self.batch_size_negs, embedding_size]))
        ...
        with tf.name_scope("mrr"):
            ...
            rank = tf.concat([pred_true, pred_false], 0)
