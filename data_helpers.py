...
def build_data(name, path='./datasets'):
    ...
    valid_triples, entities_indexes, relations_indexes = load_triples_from_txt([folder + 'valid.txt'],
                                                                               entities_indexes=entities_indexes,
                                                                               relations_indexes=relations_indexes)

    test_triples, entities_indexes, relations_indexes = load_triples_from_txt([folder + 'test.txt'],
                                                                              entities_indexes=entities_indexes,
                                                                              relations_indexes=relations_indexes)

    train = np.array(list(train_triples.keys())).astype(np.int64), np.array(list(train_triples.values())).astype(
        np.float32)
    valid = np.array(list(valid_triples.keys())).astype(np.int64), np.array(list(valid_triples.values())).astype(
        np.float32)
    test = np.array(list(test_triples.keys())).astype(np.int64), np.array(list(test_triples.values())).astype(
        np.float32)
