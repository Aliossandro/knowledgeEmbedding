...
n_entities = len(
    np.unique(np.concatenate((x_train[:, 0], x_train[:, 2], x_dev[:, 0], x_dev[:, 2], x_test[:, 0], x_test[:, 2]))))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        comemb = ComplexLogistic(n_entities, n_relations, n_entities, embedding_size=FLAGS.embedding_dim,
        ...
        loss_summary = tf.summary.scalar("loss", comemb.loss)
        acc_summary = tf.summary.scalar("accuracy", comemb.accuracy)
